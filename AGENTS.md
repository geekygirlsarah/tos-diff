...
- Digest emails are split into **two sections**: `NEW_DOCUMENTS_HEADING` ("New documents we're now tracking") and `CHANGED_DOCUMENTS_HEADING` ("Documents that changed"). A `PendingNotification` whose `old_snapshot` is `None` is the document's first snapshot and lands in the new-documents section; otherwise it is a change. Sections are omitted when empty, each is sorted alphabetically, and the subject line is built by `_digest_subject()` to name only the sections actually present (e.g. "2 new documents, 1 document change").
